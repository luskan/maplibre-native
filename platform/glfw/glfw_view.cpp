        ...
        switch (key) {
            ...
            case GLFW_KEY_U: {
                ...
                if (bounds.minPitch == mbgl::util::rad2deg(mbgl::util::PITCH_MIN) &&
                    bounds.maxPitch == mbgl::util::rad2deg(mbgl::util::PITCH_DEFAULT)) {
...
void GLFWView::captureFramebuffer(const std::string& filename) {
    ...
    // Create image buffer
    mbgl::PremultipliedImage image({static_cast<uint32_t>(fbWidth), static_cast<uint32_t>(fbHeight)});
    ...
    if (file) {
        ...
        mbgl::Log::Error(mbgl::Event::General, "Failed to open file for writing: " + filename);
    }
#else
    mbgl::Log::Warning(mbgl::Event::General, "Framebuffer capture only supported with OpenGL backend");
#endif

    // Auto-snapshot mode: exit after capture
    if (autoSnapshotMode) {
        mbgl::Log::Info(mbgl::Event::General, "Snapshot complete, exiting...");
        setShouldClose();
    }
}
...
void GLFWView::render() {
    ...
    if (autoSnapshotMode && renderCallCount <= 5) {
        mbgl::Log::Info(mbgl::Event::General, "render() called #" + std::to_string(renderCallCount) +
    ...
    if (dirty && rendererFrontend) {
        ...
        if (autoSnapshotMode && fallbackCaptureRequested && !firstRenderDone) {
            ...
            mbgl::Log::Info(mbgl::Event::General, "Capturing framebuffer from render (fallback)...");
...
void GLFWView::run() {
    ...
    if (autoSnapshotMode) {
        mbgl::Log::Info(mbgl::Event::General, "Starting 10-second fallback timer for auto-snapshot...");
        autoSnapshotTimer.start(std::chrono::seconds(10), mbgl::Duration::zero(), [this]() {
            if (!firstRenderDone) {
                mbgl::Log::Info(mbgl::Event::General, "Fallback timer triggered: requesting capture on next render...");
...
void GLFWView::onDidFinishLoadingStyle() {
    ...
    if (autoSnapshotMode) {
        mbgl::Log::Info(mbgl::Event::General, "Style loaded, waiting for tiles to load...");
    }
}
...
void GLFWView::onDidBecomeIdle() {
    ...
    if (autoSnapshotMode && !readyToCapture) {
        readyToCapture = true;
        mbgl::Log::Info(mbgl::Event::General, "Map idle, tiles loaded, will capture on next render...");
    }
}
...
void GLFWView::onDidFinishRenderingFrame(const RenderFrameStatus& status) {
    ...
    if (autoSnapshotMode && readyToCapture && !firstRenderDone && status.mode == RenderMode::Full) {
        firstRenderDone = true;
        mbgl::Log::Info(mbgl::Event::General, "Frame rendered, capturing framebuffer...");
        captureFramebuffer("./snapshot.png");
    }
}
